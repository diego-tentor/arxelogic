...
> "Because of electron-electron repulsion and nuclear screening effects, orbitals with lower (n+ℓ) have lower energy. When (n+ℓ) is equal, lower ℓ wins due to reduced angular barrier."
...
That's the primary part of Madelung's rule.

But there's more: **For equal total contradiction (n+ℓ), angular contradiction is more "expensive" than radial contradiction.**

That's the secondary ordering: **higher n (lower ℓ) fills first** when n+ℓ is equal.
...
### 2. **Why does higher n fill first when (n+ℓ) is equal?**

**Standard answer:** "Nuclear penetration—lower ℓ orbitals get closer to the nucleus."
...
- This barrier prevents nuclear approach more strongly than radial separation does
- **Lower ℓ (thus higher n for same n+ℓ) = weaker barrier = better penetration = lower energy**

**Example for n+ℓ=5:**
- **5s** (n=5, ℓ=0): Farthest shell, but **no angular barrier** → best penetration → **fills first**
- **4p** (n=4, ℓ=1): Middle shell, **weak angular barrier** → some penetration → **fills second**
- **3d** (n=3, ℓ=2): Closest shell, but **strong angular barrier** → poor penetration → **fills last**

**Result:** 5s < 4p < 3d (energy ordering)
...
**Axiom 5:** For equal total, **higher ℓ (lower n) gives higher energy** (angular barrier dominates)
...
**Translation:** Lower energy when:
1. **Primary:** Lower (n+ℓ), OR
2. **Secondary:** Same (n+ℓ) but **higher n** (equivalently, **lower ℓ**)
...
- **Why does higher n fill first for equal (n+ℓ)?** → Because angular contradiction (ℓ) creates stronger barriers than radial separation (n)
...
## A Concrete Example: The n+ℓ=5 Case

This is where Madelung becomes non-obvious and where ArXe explanation shines:

**Three orbitals have n+ℓ=5:**

| Orbital | n | ℓ | Distance from nucleus | Angular barrier | Which fills first? |
|---------|---|---|----------------------|-----------------|-------------------|
| 3d | 3 | 2 | Closest (smallest n) | Strongest (ℓ=2) | **Last** (3rd) |
| 4p | 4 | 1 | Middle | Medium (ℓ=1) | **Middle** (2nd) |
| 5s | 5 | 0 | Farthest (largest n) | None (ℓ=0) | **First** (1st) |

**Standard intuition would say:** "3d should fill first because n=3 is smallest—it's closest to nucleus!"

**But experiment shows:** 5s → 4p → 3d

**ArXe explains:** 
- 3d is closest, BUT ℓ=2 creates **strong angular barrier** (ℓ(ℓ+1)/r² term)
- This barrier **prevents nuclear approach** more than radial distance does
- 5s is farthest, BUT ℓ=0 means **no angular barrier**—it can penetrate directly
- **Angular barrier dominates over radial proximity**

**Result:** Higher n (lower ℓ) wins for equal (n+ℓ).

This is the **deep ontological principle**: angular contradiction is more "expensive" energetically than radial separation.
...
**Critical: Part 3.4 explains why higher n fills first for equal (n+ℓ)**
...
**For those seeking only the core argument:** Read Parts I-IV (especially 3.4).  
...
4. **Orbitals with same (n+ℓ) having energies that don't order by ℓ** → would refute angular barrier dominance
5. **Superheavy elements not following predicted 8s → 5g sequence** → would refute extension to high Z
6. **Finding that for equal (n+ℓ), lower n (higher ℓ) has lower energy** → would directly refute Axiom 5
...
An earlier version of this document contained an error in the statement of the secondary ordering principle. It incorrectly stated that for equal (n+ℓ), lower n gives lower energy. The correct statement is: **for equal (n+ℓ), higher n (equivalently, lower ℓ) gives lower energy**. This has been corrected throughout, with special attention to Section 3.4 which now properly explains why angular barrier dominates over radial proximity. We thank attentive readers who identified this inconsistency.
